...
class TestDeleteBookingHTML(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        # Load the HTML file and create a BeautifulSoup object for parsing
        with open('delete_booking.html', 'r') as file:
    ...
    def test_title(self):
        # Check if the title contains "Delete Booking"
        title = self.soup.title.string
        self.assertEqual(title, 'Delete Booking', "The title is incorrect")

    def test_booking_details(self):
        # Check if the booking date, time, and service name are present
        date_placeholder = self.soup.find(text="{{ booking.date }}")
        time_placeholder = self.soup.find(text="{{ booking.time }}")
        service_placeholder = self.soup.find(text="{{ booking.name }}")

        self.assertIsNotNone(date_placeholder, "Booking date placeholder is missing")
        self.assertIsNotNone(time_placeholder, "Booking time placeholder is missing")
        self.assertIsNotNone(service_placeholder, "Booking service name placeholder is missing")
    ...
    def test_submit_button(self):
        ...
        self.assertEqual(submit_button.text.strip(), 'Delete Booking', "Submit button text is incorrect")
